...
**Notes for teacher**
- Accept `const` or `var` where `let` is used (as long as the code is correct).  
- Slight variations in spacing, quotes (`'` vs `"`), or property order are fine.  
- Students should later verify their own solutions by running them with Node.js.  
- Difficulty increases gradually: pure primitives → objects/arrays → nested combinations.
